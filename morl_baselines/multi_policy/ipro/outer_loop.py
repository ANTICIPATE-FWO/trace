...
from morl_baselines.single_policy.ser.nl_mo_ppo import NLMOPPO, Agent
...
class OuterLoop(MOAgent):
    ...
    def get_pareto_set(self, subsolutions: list[Subsolution]) -> list[tuple[np.ndarray, Any]]:
        ...
        for subsolution in subsolutions:
            if np.any(np.all(np.isclose(subsolution[1], self.pf), axis=1)):
                pareto_set.append((self.sign * subsolution[1], subsolution[2], subsolution[3]))
        return pareto_set
    ...
    def replay(self, vec: np.ndarray, sol: Any, traj:Any, iter_pairs: list[Subsolution]) -> list[Subsolution]:
        ...
        for old_subproblem, old_vec, old_sol, old_traj in iter_pairs:  # Replay the points that were added correctly
            idx += 1
            if strict_pareto_dominates(old_vec, old_subproblem.referent):
                if strict_pareto_dominates(vec, old_vec):
                    ...
                    new_subsolutions.append((old_subproblem, vec, sol, traj))
                    break
                else:
                    self.update_found(old_subproblem, old_vec)
                    new_subsolutions.append((old_subproblem, old_vec, old_sol, old_traj))
            else:
                if strict_pareto_dominates(vec, old_subproblem.referent):
                    ...
                    new_subsolutions.append((old_subproblem, old_vec, old_vec, old_traj))

        for old_subproblem, old_vec, old_sol, old_traj in iter_pairs[
            idx:
            ...
            for item in items:
                ...
                if res:
                    new_subsolutions.append((res, old_vec, old_sol, old_traj))
                    break
    ...
    def train(
        ...
        while not self.is_done(iteration):
            ...
            vec, sol, traj = self.oracle_train(
