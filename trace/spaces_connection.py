...
from trace.core import  TrajectoryManager, synthetic_stochastic_points
...
plot_directory = "plots/spectral_medoids"
...
def main():
    ...
    for c in range(len(c_obs)):
        ...
        color = colors[0][c]
        fig = grid_map(c_obs[c], title=title, color=color)
        if save: fig.savefig(os.path.join(plot_directory, f"grid_map_cluster{c}.png"))
        if show: fig.show()
        fig = grid_arrows(policy, title=title, color=color)
        if save: fig.savefig(os.path.join(plot_directory, f"grid_arrows_cluster{c}.png"))
        if show: fig.show()
    ...
    if save: fig.savefig(os.path.join(plot_directory, "grid_arrows_universal.png"))
    if show: fig.show()
    fig = grid_map(un_obs[0], title=f'Universal Policy: {len(un_obs[0])} episodes', color='white')
    if save: fig.savefig(os.path.join(plot_directory, "grid_map_universal.png"))
    if show: fig.show()
